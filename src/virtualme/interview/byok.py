...
async def run_byok_gate(
    ...
    if not has_consent(keys_dir, interviewee_id):
        if candidate == CONSENT_TEXT:
            store_consent(keys_dir, interviewee_id)
            logger.info("BYOK gate: consent accepted for %s", interviewee_id)
            return GateResult(reply=CONSENT_ACCEPTED_REPLY, api_key=None)
        logger.info("BYOK gate: consent prompt sent to %s", interviewee_id)
        return GateResult(reply=CONSENT_REPLY, api_key=None)
